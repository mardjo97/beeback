...
public class ExaminationHiveResource {
  ...
  private final ExaminationHiveService service;

  @Autowired
  SecurityService securityService;

  public ExaminationHiveResource(ExaminationHiveService examinationHiveService) {
    this.service = examinationHiveService;
  }

  @PostMapping("/sync")
  public ResponseEntity<List<ExaminationHiveDTO>> syncEntities(@RequestBody List<ExaminationHiveDTO> entityDTOs, @RequestHeader("Device-Id") String deviceId) throws URISyntaxException {
    LOG.debug("REST request to sync Queens : {}", entityDTOs);
    securityService.checkUserDeviceId(deviceId);
    List<ExaminationHiveDTO> syncedEntityDTOs = service.sync(entityDTOs);
    return ResponseEntity.ok().body(syncedEntityDTOs);
  }

  @GetMapping("/all")
  public ResponseEntity<List<ExaminationHiveDTO>> userEntities(@RequestHeader(value = "Device-Id", required = false) String deviceId) {
    LOG.debug("REST request to retrieve user's Queens");
    securityService.checkUserDeviceId(deviceId);
    List<ExaminationHiveDTO> userEntityDTOs = service.userEntities(deviceId);
...
//        }
//        examinationHiveDTO = service.save(examinationHiveDTO);
...
//    }
//
//    examinationHiveDTO = service.update(examinationHiveDTO);
...
//    }
//
//    Optional<ExaminationHiveDTO> result = service.partialUpdate(examinationHiveDTO);
//
...
//    );
  ...
  public ResponseEntity<List<ExaminationHiveDTO>> getAllExaminationHives(
    ...
    Page<ExaminationHiveDTO> page = service.findAll(pageable);
  ...
  public ResponseEntity<ExaminationHiveDTO> getExaminationHive(@PathVariable("id") Long id) {
    ...
    Optional<ExaminationHiveDTO> examinationHiveDTO = service.findOne(id);
