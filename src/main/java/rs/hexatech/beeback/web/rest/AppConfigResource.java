...
public class AppConfigResource {
  ...
  @PostMapping("/sync")
  public ResponseEntity<List<AppConfigDTO>> syncEntities(@RequestBody List<AppConfigDTO> entityDTOs, @RequestHeader("Device-Id") String deviceId) throws URISyntaxException {
    LOG.debug("REST request to sync Queens : {}", entityDTOs);
    securityService.checkUserDeviceId(deviceId);
    List<AppConfigDTO> syncedEntityDTOs = service.sync(entityDTOs);
    return ResponseEntity.ok().body(syncedEntityDTOs);
  }

  @GetMapping("/all")
  public ResponseEntity<List<AppConfigDTO>> userEntities(@RequestHeader(value = "Device-Id", required = false) String deviceId) {
    LOG.debug("REST request to retrieve user's Queens");
    securityService.checkUserDeviceId(deviceId);
    List<AppConfigDTO> userEntityDTOs = service.userEntities(deviceId);
...
//    }
//    appConfigDTO = service.save(appConfigDTO);
...
//    }
//
//    appConfigDTO = service.update(appConfigDTO);
...
//    }
//
//    Optional<AppConfigDTO> result = service.partialUpdate(appConfigDTO);
//
...
//    );
  ...
  public ResponseEntity<List<AppConfigDTO>> getAllAppConfigs(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
    ...
    Page<AppConfigDTO> page = service.findAll(pageable);
  ...
  public ResponseEntity<AppConfigDTO> getAppConfig(@PathVariable("id") Long id) {
    ...
    Optional<AppConfigDTO> appConfigDTO = service.findOne(id);
