...
public class GroupResource {
  ...
  @PostMapping("/sync")
  public ResponseEntity<List<GroupDTO>> syncEntities(@RequestBody List<GroupDTO> entityDTOs, @RequestHeader("Device-Id") String deviceId) throws URISyntaxException {
    LOG.debug("REST request to sync Queens : {}", entityDTOs);
    securityService.checkUserDeviceId(deviceId);
    List<GroupDTO> syncedEntityDTOs = service.sync(entityDTOs);
    return ResponseEntity.ok().body(syncedEntityDTOs);
  }

  @GetMapping("/all")
  public ResponseEntity<List<GroupDTO>> userEntities(@RequestHeader(value = "Device-Id", required = false) String deviceId) {
    LOG.debug("REST request to retrieve user's Queens");
    securityService.checkUserDeviceId(deviceId);
    List<GroupDTO> userEntityDTOs = service.userEntities(deviceId);
...
//    );
  ...
  public ResponseEntity<List<GroupDTO>> getAllGroups(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
    ...
    Page<GroupDTO> page = service.findAll(pageable);
  ...
  public ResponseEntity<GroupDTO> getGroup(@PathVariable("id") Long id) {
    ...
    Optional<GroupDTO> groupDTO = service.findOne(id);
